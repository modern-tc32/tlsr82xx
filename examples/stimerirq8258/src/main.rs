...
tlsr82xx_hal::define_ram_void_handler! {
    unsafe extern "C" fn system_timer_led_irq() {
...
pub extern "C" fn main() -> i32 {
    ...
    timer::register_system_timer_irq_callback(tlsr82xx_hal::ram_void_handler!(
        system_timer_led_irq
    ));
    interrupt::enable();
...
fn init_led_gpio_raw() {
    unsafe {
        let ds = analog::read(ANA_GPIO_DS) | (PB4_MASK | PB5_MASK);
        analog::write(ANA_GPIO_DS, ds);
