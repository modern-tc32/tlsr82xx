...
fn main() {
    ...
    // PM retention wake on 8258 needs the runtime stack in retained SRAM.
    // The generic example linker parks `_stack_end_` at 0x850000, which is
    // outside low16K/low32K retention windows and can break wake even when
    // the PM register sequence matches vendor code.
    let local_linker = manifest_dir.join("boot_8258_ret16_stack.link");
