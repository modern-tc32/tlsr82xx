...
pub struct Board {
    ...
    pub button1: Option<PA7<Input>>,
    button1_pin: Option<PA7<Input>>,
}
...
impl Board {
    ...
    pub fn enable_button1(&mut self) {
        if self.button1.is_some() {
            return;
        }

        let Some(pin) = self.button1_pin.take() else {
            return;
        };

        let mut pin = pin.into_input();
        pin.set_pull_resistor(Pull::PullUp1M);
        self.button1 = Some(pin);
    }

    pub fn button1_pressed(&mut self) -> bool {
        match self.button1.as_mut() {
            Some(button1) => InputPin::is_low(button1).unwrap_or(false),
            None => false,
        }
    }
}
